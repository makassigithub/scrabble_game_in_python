...
Équipe 90: Miguel Castelo
           Brahima Traore
"""
from tkinter import Canvas, CENTER, Tk
class Jeton:
    """
    ...
    def __init__(self, lettre, valeur):
        ...
        # Validation des arguments passés dans le constructeur.
        assert len(lettre) == 1 and lettre.isupper() , "Lettre incorrecte."
        assert 0 <= valeur <= 20, "La valeur doit se situer entre 0 et 20 inclusivement."  # Validation du paramètre valeur.
...
class Case:
    ...
    def __init__(self, multiplicateur=1, type=None):
        ...
        # Validation des arguments passés dans le constructeur.
        assert 1 <= multiplicateur <= 3, "Le multiplicateur doit être entre 1 et 3 inclusivement."  # Validation de multiplicateur
        assert type in [None, 'M', 'L'], "Le type doit être None, 'M' ou 'L'."  # Validation de type
    ...
    def est_vide(self):
        ...
        # Retourne le résultat de la vérification jeton_occupant est None ou pas.
    ...
    def placer_jeton(self, jeton):
        ...
        assert self.est_vide(), "La case doit être vide."
    ...
    def retirer_jeton(self):
        ...
        # Vérifier si la case est vide
        assert not self.est_vide(), "Aucun jeton ne se trouve dans cette case."

        # Copier le jeton à retirer dans un nouveau objet Jeton
    ...
    def valeur_jeton(self):
        ...
        # Retourner la valeur du jeton dans la case
    ...
    def lettre_jeton(self):
        ...
        # Retourner la lettre du jeton dans la case
...
class Plateau(Canvas):
    ...
    def decode_position(code):
        ...
        assert Plateau.code_position_est_valide(code), "Le code de position n'est pas valide"

        # Convertir le code en majuscules
        code = code.upper()

        # Décoder le code de positionnement en utilisant les codes ASCII
        index_ligne = ord(code[0]) - ord('A')
        index_colonne = int(code[1:]) - 1

        # Retourner un tuple index ligne et colonne
    ...
    def case_est_vide(self, position_code):
        ...
        # Vérifier que la position est valide
        assert Plateau.code_position_est_valide(position_code), "Le code de position n'est pas valide"

        # Décoder la ligne et la colonne à partir du code position
        index_ligne, index_colonne = Plateau.decode_position(position_code)

        # Retourner True si la case est vide, sinon False
    ...
    def est_vide(self):
        ...
        # Itérer dans le plateau pour vérifier si les cases sont vides.
        for ligne in range(Plateau.DIMENSION):
            for colonne in range(Plateau.DIMENSION):
                # Si une case non-vide est trouvée, retourner False.
                if not self.cases[ligne][colonne].est_vide():
                    return False

        # Retourner True si toutes les cases sont vides.
        return True
    ...
    def ajouter_jeton(self, jeton, position_code):
        ...
        # Vérifier que la position est valide.
        assert Plateau.code_position_est_valide(position_code) and self.case_est_vide(position_code), "Le code de position n'est pas vide ou invalide"

        # Décoder les index ligne et colonne.
        index_ligne, index_colonne = Plateau.decode_position(position_code)

        # Ajouter le jeton dans la case à la position spécifiée.
    ...
    def retirer_jeton(self, position_code):
        ...
        # Vérifier que la position est valide.
        assert Plateau.code_position_est_valide(position_code) and not self.case_est_vide(position_code), "Le code de position est vide ou invalide"

        # Décoder les index ligne et colonne.
        index_ligne, index_colonne = Plateau.decode_position(position_code)

        # Retirer et retourner le jeton.
    ...
    def placer_mots(self, jetons_a_ajouter, position_codes):
        ...
        # Retourner le tuple des listes de mots formés et du score obtenu.
