...
from tkinter import Tk,Canvas,NSEW,W, Label, Frame,Button
...
class Scrabble(Tk):
    ...
    def mot_permis(self, mot):
        ...
        # Vérifier si le mot est dans le dictionnaire et retourner la valeur du booléen
    ...
    def determiner_gagnant(self):
        ...
        # Trier la liste en ordre croissant de points.
        liste_triee = sorted(self.joueurs, key=lambda joueur: joueur.points)

        # Déterminer un joueur avec le pointage le plus élevé.
        gagnant = liste_triee[-1]

        # Vérifier si d'autres joueurs ont le même pointage maximal.
        liste_gagants = []
        liste_gagants.append(gagnant)

        # Itérer sur la liste des joueurs et créer une liste avec tous les joueurs potentiellement gagnants.
        for i in range(len(liste_triee)):
            if self.joueurs[i].points == gagnant.points:
                liste_gagants.append(self.joueurs[i])

        # Si un seul joueur a le pointage maximal, retourner le joueur.
        if len(liste_gagants) == 1:
            return gagnant

        # Sinon retourner au hasard un des joueur ayant le pointage maximal.
        return liste_gagants[randint(0, len(liste_gagants) - 1)]
    ...
    def partie_terminee(self):
        ...
        # Terminer la partie s'il ne reste plus de jetons libres ou s'il reste moins que 2 joueurs.
    ...
    def joueur_suivant(self):
        ...
        # S'il n'y a pas de joueur actif, en choisir un au hasard
        if self.joueur_actif is None:
            self.joueur_actif = self.joueurs[randint(0, len(self.joueurs) - 1)]
        else:
            # Sinon passer au joueur suivant
            index_joueur = (self.joueurs.index(self.joueur_actif) + 1) % len(self.joueurs)
            self.joueur_actif = self.joueurs[index_joueur]
    ...
    def tirer_jetons(self, n):
        ...
        # Vérifier que la valeur de n est comprise entre 0 et 7 inclusivement.
        assert 0 <= n <= 7, "La valeur de n doit être comprise entre 0 et 7 inclusivement."

        # Créer une liste de jetons à retourner.
        liste_jetons = []

        # S'assurer qu'on ne tente pas de tirer plus de jetons que le nombre de jetons libres restants.
        if len(self.jetons_libres) < 7:
            n = len(self.jetons_libres)

        # Mélanger la liste de jetons libres.
        shuffle(self.jetons_libres)

        # Itérer dans la liste de jetons mélanger et tirer n jetons.
        for i in range(n):
            liste_jetons.append(self.jetons_libres.pop())

        # Retourner la liste de jetons tirés.
        return liste_jetons
    ...
    def changer_jetons(self):
        ...
        # Demander au joueur la liste de jetons à changer.
        position_a_echanger = [int(x) - 1 for x in
                               input("Entrez les positions à échanger separés d'espace: ").strip().split(' ')]

        # pour qu'un changement soit possible, il faut les positions specifées soient valides et qu'il y ait un nombre suffisant de jetons libres
        changement_possible = False

        # Valider et redemander au besoin la liste des jetons à changer.
        while not changement_possible:
            # Toute les positions doivent etre valides si non on reprend
            if any([not Joueur.position_est_valide(pos) for pos in
                    position_a_echanger]):
                position_a_echanger = [int(x) for x in
                                       input("Une ou plusieurs position(s) sont invalide(s). Reéssayez: ").strip().split(' ')]

            # Il faut un nombre suffisant de jetons libres
            if len(position_a_echanger) >= len(self.jetons_libres):
                position_a_echanger = [int(x) for x in input(
                    "Vous ne pouver echanger qu'au plus {} jeton(s). Reéssayez: ".format(
                        len(self.jetons_libres))).strip().split(' ')]

            # Réevaluation des conditions citées, et retour dans la boucle au cas echéant
            changement_possible = len(position_a_echanger) <= len(self.jetons_libres) and all(
                [Joueur.position_est_valide(pos) for pos in position_a_echanger])

        # Les jetons retirés sont stockés dans jetons_retires
        jetons_retires = list()
        for pos in position_a_echanger:
            jetons_retires.append(self.joueur_actif.retirer_jeton(pos))

        # Les nouveaux jetons sont stocké
        nouveaux_jetons = self.tirer_jetons(len(position_a_echanger))

        # Les nouveaux jetons sont ajouté aux chevalet du joueur
        for jeton in nouveaux_jetons:
            self.joueur_actif.ajouter_jeton(jeton)

        # Remettre les jetons retirés dans le sac
        for jeton in jetons_retires:
            self.jetons_libres.append(jeton)
